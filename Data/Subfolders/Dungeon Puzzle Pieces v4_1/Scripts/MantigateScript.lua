...
local propStaysOpen = script.parent:GetCustomProperty("StaysOpen")
...
function CloseTheGate()
	if (gateState == GATESTATE_OPEN) then
		propGate:MoveTo(startingPos, GateRaiseTime)
		...
		GateSoundTask = Task.Spawn(
			function()
				local startTime = time()
				while time() < startTime + GateRaiseTime do
					propGateLowerSound:Play()
					Task.Wait(0.3)
				end
			end
...
function OnTriggerUp(TriggerRef)
	if TriggerRef:GetObject().parent == script.parent.parent then
		...
		if not propStaysOpen and CountButtonsDown() < propRequiredButtonCount then
			CloseTheGate()
		end		
	end
end
